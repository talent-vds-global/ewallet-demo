...
| Dashboard db-quality báo `HikariDataSource (null) has been closed` hoặc trả rỗng khi chạy trên host | Cả 4 service có DB đều đặt `quality.dashboard.port=9876`. Chạy nhiều service trên host thì chúng tranh nhau cùng một cổng (trong Docker không dính vì mỗi container có network riêng, ánh xạ ra 19082–19085) | Chạy trong Docker để xem db-quality, hoặc tạm đặt `quality.dashboard.enabled=false` ở các service không cần soi. Thư viện chỉ đọc `application.properties` trên classpath nên không override được bằng tham số dòng lệnh |
